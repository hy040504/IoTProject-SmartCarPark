...
| `entrance_sensor.h` | 입구 조도센서 감지, 차단기 통과 감지 함수 제공 |
| `parking_slots.h` | 주차칸 2개 상태 확인, 칸별 LED 제어, 입차/출차 대기 처리 |
| `parking_display.h` | I2C LCD 메시지 출력 |
| `parking_alert.h` | 만차 경고 LED/부저 제어 |
| `barrier.h` | 서보모터 기반 차단기 열림/닫힘 |
| `fee_request.h` | Wemos에서 요금 서버로 HTTP GET 요청 전송 |
| `fee-server/src/server.js` | Express API 서버 |
| `fee-server/src/feeCalculator.js` | 주차 요금 계산 로직 |
...
| 장치 | 핀 | 위치 |
| --- | --- | --- |
| 입구 조도센서 | `A0` | `entrance_sensor.h` |
| 차단기 서보모터 | `D5` | `barrier.h` |
| 만차 경고 LED | `D0` | `parking_alert.h` |
| 만차 경고 부저 | `D8` | `parking_alert.h` |
| 1번 칸 초음파 TRIG | `D6` | `parking_slots.h` |
| 1번 칸 초음파 ECHO | `D3` | `parking_slots.h` |
| 2번 칸 초음파 TRIG | `D7` | `parking_slots.h` |
| 2번 칸 초음파 ECHO | `D4` | `parking_slots.h` |
| 1번 칸 빨간 LED | `D9` | `parking_slots.h` |
| 1번 칸 초록 LED | `D10` | `parking_slots.h` |
| 2번 칸 빨간 LED | `D11` | `parking_slots.h` |
| 2번 칸 초록 LED | `D12` | `parking_slots.h` |
| I2C LCD | SDA / SCL | `parking_display.h` |
...
`fee_request.h`에서 Wi-Fi와 서버 주소를 수정합니다.
...
- I2C LCD 주소가 `0x27`이 아니면 `parking_display.h`의 `LCD_ADDRESS` 값을 바꿔야 합니다.
